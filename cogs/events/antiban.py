        ...
        try:
            ...
            async for entry in guild.audit_logs(
                limit=1):
                  ...
                  if entry.user == guild.owner or str(entry.user.id) in wled or anti == "off" or antiban == False or user == self.client.user.id:
                    pass
    ...
    async def on_member_unban(self, guild: discord.Guild, user: discord.User) -> None:
        try:
            ...
            async for entry in guild.audit_logs(
              ...
              use = entry.user.id
              api = random.randint(8,9)
              if entry.user.id == self.client.user.id or entry.user.id == guild.owner_id or str(entry.user.id) in wled or anti == "off" or antiban == False:
               return
