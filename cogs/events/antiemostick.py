...
class antiemostick(Cog):
    ...
    async def on_guild_emojis_update(self, guild, before, after) -> None:
      try:
        ...
        antievent = event["antinuke"]["antiemoji-create"]
        ...
        if str(entry.user.id) in wled or anti == "off" or user == self.client.user.id or antievent == False or entry.user == guild.owner:
            pass
        else:
